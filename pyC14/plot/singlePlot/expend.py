...
def plot(ocd,
    ...
    max_prob = max(calibrated.array[:,1])
    min_prob = min(calibrated.array[:,1])

    ax2.fill(
        calibrated.array[:,0],
        calibrated.array[:,1] + max_prob*0.3,
        'k',
        alpha=0.3,
        label='Calendar Age'
        )
    ...
    ax2.set_ybound(min_prob, max_prob*3)
    ax2.set_axis_off()
    ...
    ax3 = plt.twiny(ax1)
    for i in range(-1,2,1):     # ==> i in [-1, 0, 1]
        ax3.plot(
            line_x,
            line_y+i*ocd.error,
            'k--',
